...
| 4 | **REVISED 2026-05-19**: Switch to OpenAI `text-embedding-3-small` (1536-d) + Claude Sonnet 4.6 generation + Claude Haiku 4.5 for structured / judge / query-rewriting | The original "keep Gemini for embedding + generation" preserved exactly the vendor coupling we set out to eliminate. Full pivot: OpenAI embeddings (vendor #3, recoverable offline if dropped), Anthropic generation (vendor #1, same as P3 extraction so single key to rotate), no Gemini anywhere in the OG-RAG hot path. Cost goes from ~$15/mo to ~$450/mo at 1000 queries/day (Sonnet ~30× Flash). Existing 66 seeded chunks at 768-d must be re-embedded at 1536-d. Existing vector(768) columns dropped+recreated as vector(1536) via migration 008. |
